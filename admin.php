...
if (!isset($_SERVER['PHP_AUTH_USER']) || !isset($_SERVER['PHP_AUTH_PW'])) {
    header('WWW-Authenticate: Basic realm="Админ-панель Задание 7"');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_id'])) {
    ...
    if ($has_error) {
        ...
        try {
            ...
            $pdo->rollBack();
            error_log("Database error: " . $e->getMessage());
            $messages[] = '<div class="error-message">Ошибка при сохранении. Попробуйте позже.</div>';
        }
    }
}
...
<div class="container">
    ...
    <?php if ($edit_id > 0 && !empty($edit_values)): ?>
        ...
        <form method="POST" style="background:#252525;padding:25px;border-radius:15px;">
            ...
            <div class="form-group">
                <label>ФИО</label>
                <input type="text" name="full_name" value="<?= htmlspecialchars($edit_values['full_name'] ?? '') ?>" class="<?= isset($edit_errors['full_name']) ? 'error-field' : '' ?>">
                <?php if (isset($edit_errors['full_name'])): ?><span class="field-error"><?= $edit_errors['full_name'] ?></span><?php endif; ?>
            </div>

            <div class="form-group">
                <label>Телефон</label>
                <input type="tel" name="phone" value="<?= htmlspecialchars($edit_values['phone'] ?? '') ?>" class="<?= isset($edit_errors['phone']) ? 'error-field' : '' ?>">
                <?php if (isset($edit_errors['phone'])): ?><span class="field-error"><?= $edit_errors['phone'] ?></span><?php endif; ?>
            </div>

            <div class="form-group">
                <label>E-mail</label>
                <input type="email" name="email" value="<?= htmlspecialchars($edit_values['email'] ?? '') ?>" class="<?= isset($edit_errors['email']) ? 'error-field' : '' ?>">
                <?php if (isset($edit_errors['email'])): ?><span class="field-error"><?= $edit_errors['email'] ?></span><?php endif; ?>
            </div>

            <div class="form-group">
                <label>Дата рождения</label>
                <input type="date" name="birth_date" value="<?= htmlspecialchars($edit_values['birth_date'] ?? '') ?>" class="<?= isset($edit_errors['birth_date']) ? 'error-field' : '' ?>">
                <?php if (isset($edit_errors['birth_date'])): ?><span class="field-error"><?= $edit_errors['birth_date'] ?></span><?php endif; ?>
            </div>
            ...
            <div class="form-group">
                ...
                <?php if (isset($edit_errors['gender'])): ?><span class="field-error"><?= $edit_errors['gender'] ?></span><?php endif; ?>
            </div>
            ...
            <div class="form-group">
                ...
                <?php if (isset($edit_errors['languages'])): ?><span class="field-error"><?= $edit_errors['languages'] ?></span><?php endif; ?>
            </div>
            ...
            <div class="form-group">
                ...
                <?php if (isset($edit_errors['biography'])): ?><span class="field-error"><?= $edit_errors['biography'] ?></span><?php endif; ?>
            </div>

            <div class="form-group checkbox">
                <label>
                    <input type="checkbox" name="contract_accepted" value="1" <?= !empty($edit_values['contract_accepted']) ? 'checked' : '' ?> class="<?= isset($edit_errors['contract_accepted']) ? 'error-field' : '' ?>">
                    Я ознакомлен(а) с контрактом
                </label>
                <?php if (isset($edit_errors['contract_accepted'])): ?><span class="field-error"><?= $edit_errors['contract_accepted'] ?></span><?php endif; ?>
            </div>
    ...
    <tr>
        ...
        <td><a href="admin.php?edit=<?= $app['id'] ?>">✏️ Ред.</a> | <a href="admin.php?delete=<?= $app['id'] ?>" onclick="return confirm('Удалить анкету №<?= $app['id'] ?>?')">🗑 Удалить</a></td>
    </tr>
    <?php endforeach; ?>
    ...
    <div style="text-align:center;margin-top:40px;">
        ...
        <a href="audit_security.html" style="margin-left:15px;">🔒 Аудит безопасности</a>
    </div>
</div>
</body>
</html>