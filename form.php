...
<head>
    ...
    <style>
        .bottom-links { margin-top: 20px; text-align: center; display: flex; justify-content: center; gap: 15px; flex-wrap: wrap; }
        .bottom-links a { display: inline-block; background-color: #39e704; color: white; text-decoration: none; padding: 10px 25px; border-radius: 5px; font-weight: bold; }
        .logged-in { background: #e6f7e6; color: #155724; padding: 10px; border-radius: 5px; text-align: center; margin-bottom: 15px; }
        .auth-hint { text-align: center; margin-top: 15px; color: #888; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <?php if ($is_logged_in): ?>
            <p class="logged-in">✅ Вы авторизованы (ID: <?= htmlspecialchars($user_id) ?>) <a href="index.php?logout=1" style="color:#155724; margin-left:15px;">Выйти</a></p>
        <?php endif; ?>

        <?php if (!empty($messages)): ?>
            <div class="messages"><?php foreach ($messages as $msg) echo $msg; ?></div>
        <?php endif; ?>
        ...
        <form method="post" action="index.php">
            ...
            <div class="form-group">
                ...
                <?php if (!empty($errors['languages'])): ?><span class="field-error">Выберите хотя бы один допустимый язык</span><?php endif; ?>
            </div>

            <div class="form-group">
                <label for="biography">Биография:</label>
                <textarea id="biography" name="biography" rows="6" <?= !empty($errors['biography']) ? 'class="error"' : '' ?>><?= htmlspecialchars($values['biography'] ?? '') ?></textarea>
                <?php if (!empty($errors['biography'])): ?><span class="field-error">Биография слишком длинная</span><?php endif; ?>
            </div>

            <div class="form-group checkbox">
                <label><input type="checkbox" name="contract_accepted" value="1" <?= !empty($values['contract_accepted']) ? 'checked' : '' ?> <?= !empty($errors['contract_accepted']) ? 'class="error"' : '' ?>> Я ознакомлен(а) с контрактом</label>
                <?php if (!empty($errors['contract_accepted'])): ?><span class="field-error">Необходимо подтвердить согласие</span><?php endif; ?>
            </div>

            <div class="form-group"><button type="submit"><?= $is_logged_in ? 'Сохранить изменения' : 'Сохранить' ?></button></div>
        </form>
        ...
        <?php if (!$is_logged_in): ?><div class="auth-hint"><small>(Для редактирования данных нужна авторизация)</small></div><?php endif; ?>
    </div>
</body>
</html>